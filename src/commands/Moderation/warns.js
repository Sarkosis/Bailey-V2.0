...
module.exports = {
	...
	async execute(interaction, client) {
		...
		const subcgroup = options.getSubcommandGroup();
		const subc = options.getSubcommand();
		const wmember = options.getMember('member');
		const user = options.getString('user_id');
		const reason = options.getString('reason') || 'No reason provided.';
		const cid = options.getNumber('case_id');
						...
						case 'user':
							{
								await client.connection.query(
									...
									async (er, results) => {
										...
										const caseid = results[0]?.total + 1;
										console.log(user);
										const theuser = await client.users.fetch(user, {
											force: true,
										});
