      ...
      async (e, mrows) => {
        ...
        await client.con.query(
          ...
          async (e, brows) => {
            ...
            await client.con.query(
              `SELECT * FROM logs WHERE GuildID = '${guild.id}' AND ChannelID = "${ticket.id}" AND Locked = "true"`,
              async (e, row) => {
                if (e) {
                  if (client.config.debugmode) {
                    console.log(e.stack);
                ...
                if (!row[0]) {
                  return interaction.reply({
                    content: "Ticket not found!\n**Notice: This could be because the ticket isn;t locked.**",
                    ephemeral: true,
                  });
                }
                await client.con.query(
                  `UPDATE logs SET Locked = "false" WHERE GuildID = '${guild.id}' AND ChannelID = "${ticket.id}"`,
                  async (e) => {
                    if (e) {
                      if (client.config.debugmode) {
                        console.log(e.stack);
                      ...
                      await thechan.send({
                        embeds: [
                          new EmbedBuilder()
                            ...
                            .setDescription(
                              `${member.user.tag} has unlocked this ticket.`
                            ),
                        ],
                      });
                    ...
                    return interaction.reply({
                      content: "Successfully unlocked ticket!",
                      ephemeral: true,
                    });
                  }
                );
