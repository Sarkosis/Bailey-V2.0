      ...
      async (e, mrows) => {
        ...
        await client.con.query(
          ...
          async (e, brows) => {
            ...
            await client.con.query(
              `SELECT * FROM logs WHERE GuildID = '${guild.id}' AND ChannelID = "${ticket.id}" AND Closed = "false"`,
              async (e, row) => {
                if (e) {
                  if (client.config.debugmode) {
                    console.log(e.stack);
                ...
                if (!row[0]) {
                  return interaction.reply({
                    content: "Ticket not found!\n**Notice: This could be because the ticket is already closed.**",
                    ephemeral: true,
                  });
                }
                await client.con.query(
                  `UPDATE logs SET Closed = "true" WHERE GuildID = '${guild.id}' AND ChannelID = "${ticket.id}"`,
                  async (e) => {
                    if (e) {
                      if (client.config.debugmode) {
                        console.log(e.stack);
                      ...
                      await thechan.send({
                        embeds: [
                          new EmbedBuilder()
                            ...
                            .setDescription(
                              `${member.user.tag} has closed this ticket.`
                            ),
                        ],
                      });
                      setTimeout(async () => {
                        await thechan.delete()
                      }, 5000);
                    } catch (error) {
                      return interaction.reply({
