      ...
      async (e, mrows) => {
        ...
        await client.con.query(
          ...
          async (e, brows) => {
            ...
            await client.con.query(
              `SELECT * FROM logs WHERE GuildID = '${guild.id}' AND ChannelID = "${ticket.id}" AND ClaimingMemberID  = "none"`,
              async (e, row) => {
                if (e) {
                  if (client.config.debugmode) {
                    console.log(e.stack);
                ...
                if (!row[0]) {
                  return interaction.reply({
                    content: "Ticket not found!\n**Notice: This could be because the ticket is already claimed.**",
                    ephemeral: true,
                  });
                }
                await client.con.query(
                  `UPDATE logs SET ClaimingMemberID = "${member.id}" WHERE GuildID = '${guild.id}' AND ChannelID = "${ticket.id}"`,
                  async (e) => {
                    if (e) {
                      if (client.config.debugmode) {
                        console.log(e.stack);
                    ...
                    return thechan.send({
                      embeds: [
                        new EmbedBuilder()
                          ...
                          .setDescription(
                            `${member.user.tag} has claimed <#${thechan.id}>`
                          ),
                      ],
                    });
                  }
