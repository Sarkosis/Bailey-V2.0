...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("unarchive")
    .setDescription("Unarchive a ticket")
    .addChannelOption((o) =>
      o
        .setName("ticket")
    ...
    .addChannelOption((o) =>
      o
        .setName("category")
        .setDescription("Input the new category")
      ...
      async (e, mrows) => {
        ...
        await client.con.query(
          ...
          async (e, brows) => {
            ...
            await client.con.query(
              `SELECT * FROM logs WHERE GuildID = '${guild.id}' AND ChannelID = "${ticket.id}" AND Archived = "true"`,
              async (e, row) => {
                if (e) {
                  if (client.config.debugmode) {
                    console.log(e.stack);
                ...
                if (!row[0]) {
                  return interaction.reply({
                    content: "Ticket not found!\n**Notice: This could be because the ticket isn't archived.**",
                    ephemeral: true,
                  });
                }
                await client.con.query(
                  `UPDATE logs SET Archived = "false" WHERE GuildID = '${guild.id}' AND ChannelID = "${ticket.id}"`,
                  async (e) => {
                    if (e) {
                      if (client.config.debugmode) {
                        console.log(e.stack);
                    ...
                    try {
                      await thechan.edit({
                        parent: thecat.id,
                        topic: "Ticket unarchived",
                      });
                    } catch (error) {
                      return interaction.reply({
                        content: "Failed to unarchive",
                        ephemeral: true,
                      });
                    }
                    return interaction.reply({
                      content: "Successfully unarchived ticket!",
                      ephemeral: true,
                    });
                  }
                );
